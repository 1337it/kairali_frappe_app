...
def item_query(doctype, txt, searchfield, start, page_len, filters, as_dict=False):
	...
	return frappe.db.sql(
		"""select
			it.item_name, CONCAT('Item Code:', RPAD(NVL(it.item_code, '') , 15, SPACE(1))) as label, CONCAT('Description:', RPAD(NVL(it.description, '') , 50, SPACE(1))) as description, CONCAT('Price:', RPAD(NVL(format(ip.price_list_rate, 'C2'), '') , 8, SPACE(1))) as price, CONCAT('QTR:', cast((SELECT sum(sl.actual_qty) FROM `tabStock Ledger Entry` sl WHERE sl.item_code = `tabItem`.item_code AND sl.warehouse = "Stores - QTR" GROUP BY sl.item_code) AS int)) AS 'QTY'
