...
def item_query(doctype, txt, searchfield, start, page_len, filters, as_dict=False):
	...
	return frappe.db.sql(
                """select
                        tabItem.name as name, tabItem.description as description, ip.price_list_rate AS retail_price, sum(iw.actual_qty) AS available_qty, `tabStock Ledger Entry`.actual_qty AS in_store_qty, ia.alternative_item_code as substitute
			from tabItem
   ...
   LEFT OUTER JOIN `tabStock Ledger Entry` AS iw ON tabItem.item_code = `tabStock Ledger Entry`.item_code
