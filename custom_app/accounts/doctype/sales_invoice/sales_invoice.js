...
frappe.ui.form.on("Sales Invoice", {
	...
	refresh: function (frm) {
			...
			if (
				...
				frm.add_custom_button(__("Update Items"), () => {
					erpnext.utils.update_child_items({
						frm: frm,
						child_docname: "items",
						child_doctype: "Sales Invoice Detail",
		...
		if (frm.doc.docstatus === 0) {
			if (frm.doc.is_internal_customer) {
				frm.events.get_items_from_internal_purchase_Invoice(frm);
			}

			if (frm.doc.docstatus === 0) {
				frappe.call({
					method: "erpnext.selling.doctype.sales_Invoice.sales_Invoice.get_stock_reservation_status",
					callback: function (r) {
						if (!r.message) {
		...
		// Hide `Reserve Stock` field description in submitted or cancelled Sales Invoice.
		if (frm.doc.docstatus > 0) {
			frm.set_df_property("reserve_stock", "description", null);
		}
	},

	get_items_from_internal_purchase_Invoice(frm) {
		frm.add_custom_button(
			__("Purchase Invoice"),
			() => {
				erpnext.utils.map_current_doc({
					method: "erpnext.buying.doctype.purchase_Invoice.purchase_Invoice.make_inter_company_sales_Invoice",
	...
	onload: function (frm) {
		...
		// On cancel and amending a sales Invoice with advance payment, reset advance paid amount
		if (frm.is_new()) {
			frm.set_value("advance_paid", 0);
		}

		frm.ignore_doctypes_on_cancel_all = ["Purchase Invoice"];
	},
		...
		const dialog = new frappe.ui.Dialog({
			...
			fields: [
				...
				{ fieldtype: "Column Break" },
				{
					...
					onchange: () => {
						let sales_Invoice_item = dialog.get_value("add_item");

						if (sales_Invoice_item) {
							frm.doc.items.forEach((item) => {
								if (item.name === sales_Invoice_item) {
									let unreserved_qty =
										(flt(item.stock_qty) -
									...
									if (unreserved_qty > 0) {
										dialog.fields_dict.items.df.data.forEach((row) => {
											if (row.sales_Invoice_item === sales_Invoice_item) {
					...
					fields: [
						{
							...
							onchange: (event) => {
								if (event) {
									...
									frm.doc.items.forEach((item) => {
										if (item.name === item_row.sales_Invoice_item) {
...
erpnext.selling.SalesInvoiceController = class SalesInvoiceController extends erpnext.selling.SellingController {
	onload(doc, dt, dn) {
	...
	refresh(doc, dt, dn) {
			...
			if (this.frm.has_perm("submit")) {
				if (doc.status === "On Hold") {
					...
					if (flt(doc.per_delivered, 2) < 100 || flt(doc.per_billed, 2) < 100) {
						// close
						this.frm.add_custom_button(__("Close"), () => this.close_sales_Invoice(), __("Status"));
					}
				...
				if (doc.status !== "On Hold") {
					...
					if (this.frm.has_perm("submit")) {
						if (flt(doc.per_delivered, 2) < 100 || flt(doc.per_billed, 2) < 100) {
							...
							this.frm.add_custom_button(
								__("Hold"),
								() => this.hold_sales_Invoice(),
								__("Status")
							);
							// close
							this.frm.add_custom_button(
								__("Close"),
								() => this.close_sales_Invoice(),
								__("Status")
							);
						}
					}
					...
					if (!doc.__onload || !doc.__onload.has_reserved_stock) {
						// Don't show the `Reserve` button if the Sales Invoice has Picked Items.
					...
					const Invoice_is_a_sale = ["Sales", "Shopping Cart"].indexOf(doc.Invoice_type) !== -1;
					const Invoice_is_maintenance = ["Maintenance"].indexOf(doc.Invoice_type) !== -1;
					// Invoice type has been customised then show all the action buttons
					const Invoice_is_a_custom_sale =
						["Sales", "Shopping Cart", "Maintenance"].indexOf(doc.Invoice_type) === -1;

					// delivery note
					if (
						flt(doc.per_delivered, 2) < 100 &&
						(Invoice_is_a_sale || Invoice_is_a_custom_sale) &&
						allow_delivery
					) {
						this.frm.add_custom_button(
							__("Delivery Note"),
					...
					if (
						!doc.Invoice_type ||
						((Invoice_is_a_sale || Invoice_is_a_custom_sale) && flt(doc.per_delivered, 2) < 100)
					) {
						this.frm.add_custom_button(
							__("Material Request"),
					...
					// maintenance
					if (flt(doc.per_delivered, 2) < 100 && (Invoice_is_maintenance || Invoice_is_a_custom_sale)) {
						this.frm.add_custom_button(
		...
		this.Invoice_type(doc);
	}

	create_pick_list() {
		frappe.model.open_mapped_doc({
			method: "erpnext.accounts.doctype.sales_invoice.sales_invoice.create_pick_list",
			frm: this.frm,
		});
	}
	...
	make_work_Invoice() {
		var me = this;
		me.frm.call({
			method: "erpnext.selling.doctype.sales_Invoice.sales_Invoice.get_work_Invoice_items",
			args: {
				sales_Invoice: this.frm.docname,
			},
			freeze: true,
			callback: function (r) {
				if (!r.message) {
					frappe.msgprint({
						title: __("Work Invoice not created"),
					...
					var d = new frappe.ui.Dialog({
						...
						primary_action: function () {
							...
							me.frm.call({
								method: "make_work_Invoices",
								args: {
									items: data,
									company: me.frm.doc.company,
									sales_Invoice: me.frm.docname,
								...
								callback: function (r) {
									if (r.message) {
										frappe.msgprint({
											message: __("Work Invoices Created: {0}", [
												r.message
													.map(function (d) {
														return repl(
															'<a href="/app/work-Invoice/%(name)s">%(name)s</a>',
															{ name: d }
														);
													})
													.join(", "),
	...
	make_material_request() {
		frappe.model.open_mapped_doc({
			method: "erpnext.selling.doctype.sales_Invoice.sales_Invoice.make_material_request",
			frm: this.frm,
		});
	}
	...
	toggle_delivery_date() {
		this.frm.fields_dict.items.grid.toggle_reqd(
			"delivery_date",
			this.frm.doc.Invoice_type == "Sales" && !this.frm.doc.skip_delivery_note
		);
	}
	...
	make_raw_material_request_dialog(r) {
		...
		var fields = [
			...
			{
				fieldtype: "Check",
				fieldname: "ignore_existing_Invoiceed_qty",
				label: __("Ignore Existing Invoiceed Qty"),
			},
			{
				fieldtype: "Table",
				fieldname: "items",
		...
		var d = new frappe.ui.Dialog({
			...
			primary_action: function () {
				...
				me.frm.call({
					method: "erpnext.selling.doctype.sales_Invoice.sales_Invoice.make_raw_material_request",
					args: {
						items: data,
						company: me.frm.doc.company,
						sales_Invoice: me.frm.docname,
	...
	make_delivery_note_based_on_delivery_date(for_reserved_stock = false) {
		...
		if (!item_grid.get_selected().length && delivery_dates.length > 1) {
			...
			var html = $(`
				<div style="bInvoice: 1px solid #d1d8dd">
	...
	make_delivery_note(delivery_dates, for_reserved_stock = false) {
		frappe.model.open_mapped_doc({
			method: "erpnext.selling.doctype.sales_Invoice.sales_Invoice.make_delivery_note",
			frm: this.frm,
			args: {
				delivery_dates,
	...
	make_sales_invoice() {
		frappe.model.open_mapped_doc({
			method: "erpnext.selling.doctype.sales_Invoice.sales_Invoice.make_sales_invoice",
			frm: this.frm,
		});
	}
	...
	make_project() {
		frappe.model.open_mapped_doc({
			method: "erpnext.selling.doctype.sales_Invoice.sales_Invoice.make_project",
			frm: this.frm,
		});
	}
	...
	make_inter_company_Invoice() {
		frappe.model.open_mapped_doc({
			method: "erpnext.selling.doctype.sales_Invoice.sales_Invoice.make_inter_company_purchase_Invoice",
			frm: this.frm,
		});
	}
	...
	make_maintenance_visit() {
		frappe.model.open_mapped_doc({
			method: "erpnext.selling.doctype.sales_Invoice.sales_Invoice.make_maintenance_visit",
			frm: this.frm,
		});
	}
	...
	make_purchase_Invoice() {
		let pending_items = this.frm.doc.items.some((item) => {
			let pending_qty = flt(item.stock_qty) - flt(item.Invoiceed_qty);
			return pending_qty > 0;
		});
		if (!pending_items) {
			frappe.throw({
				message: __("Purchase Invoice already created for all Sales Invoice items"),
				title: __("Note"),
			});
		}
		...
		var dialog = new frappe.ui.Dialog({
			...
			primary_action(args) {
				...
				var method = args.against_default_supplier
					? "make_purchase_Invoice_for_default_supplier"
					: "make_purchase_Invoice";
				return frappe.call({
					method: "erpnext.selling.doctype.sales_Invoice.sales_Invoice." + method,
		...
		function set_po_items_data(dialog) {
			...
			if (against_default_supplier) {
				...
				me.frm.doc.items.forEach((d) => {
					let Invoiceed_qty = me.get_Invoiceed_qty(d, me.frm.doc);
					let pending_qty = (flt(d.stock_qty) - Invoiceed_qty) / flt(d.conversion_factor);
					if (pending_qty > 0) {
						po_items.push({
	...
	get_Invoiceed_qty(item, so) {
		let Invoiceed_qty = item.Invoiceed_qty;
		if (so.packed_items && so.packed_items.length) {
			// calculate Invoiceed qty based on packed items in case of product bundle
			let packed_items = so.packed_items.filter((pi) => pi.parent_detail_docname == item.name);
			if (packed_items && packed_items.length) {
				Invoiceed_qty = packed_items.reduce((sum, pi) => sum + flt(pi.Invoiceed_qty), 0);
				Invoiceed_qty = Invoiceed_qty / packed_items.length;
			}
		}
		return Invoiceed_qty;
	}
	...
	update_status(label, status) {
		...
		frappe.call({
			method: "erpnext.selling.doctype.sales_Invoice.sales_Invoice.update_status",
...
extend_cscript(cur_frm.cscript, new erpnext.selling.SalesInvoiceController({ frm: cur_frm }));
